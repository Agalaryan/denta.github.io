...
if ( function_exists('modeltheme_framework')) {
    if (isset($select_post_sidebar) && $select_post_sidebar != '') {
        ...
        $sidebar = medpluswp_redux('mt_single_blog_layout_sidebar');
    }
}
...
if (isset($select_post_layout) && in_array($select_post_layout, $sidebars_lr_meta)) {
    ...
    if(class_exists( 'ReduxFrameworkPlugin' )){
        ...
        if (in_array(medpluswp_redux('mt_single_blog_layout'), $sidebars_lr_panel)) {
...
if ( function_exists('modeltheme_framework')) {
    if (isset($breadcrumbs_on_off) && $breadcrumbs_on_off == 'yes' || $breadcrumbs_on_off == '') {
        echo medpluswp_header_title_breadcrumbs();
    }
}else{
    echo wp_kses_post(medpluswp_header_title_breadcrumbs());
}
?>
           ...
           <?php // BEGIN_WP5 ?>
            ...
            <?php }else{ ?>
                <?php if (isset($select_post_layout) && $select_post_layout == 'inherit') { ?>
                    <?php if(class_exists( 'ReduxFrameworkPlugin' )){ ?>
                        <?php if ( medpluswp_redux('mt_single_blog_layout') == 'mt_single_blog_left_sidebar') { ?>
                            <div class="col-md-4 col-sm-4 sidebar-content sidebar-left">
                                <?php if (is_active_sidebar($sidebar)) { ?>
                                    <?php dynamic_sidebar($sidebar); ?>
                                <?php } ?>
            ...
            <div class="<?php echo esc_attr($cols); ?> main-content">
                ...
                <div class="article-header">
                    <div class="article-details">
                        <?php if(has_post_thumbnail()) {
                            $thumbnail_src = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ),'medpluswp_blog_grid_1200x700' );
                            if($thumbnail_src) {
                                echo '<img src="'.esc_url($thumbnail_src[0]).'" class="img-responsive single-post-featured-img" alt="'.get_the_title().'" />';
                            }
                        } ?>
                        <div class="clearfix"></div>

                        <h3 class="post-name"><?php echo get_the_title(); ?></h3>
                        <?php echo get_the_term_list( get_the_ID(), 'category', '', ' ' ).' - '.get_the_date(get_option( 'date_format' )); ?>
                        <div class="clearfix"></div>
                ...
                <div class="article-content">
                    ...
                    <!-- AUTHOR BIO -->
                    <?php if ( medpluswp_redux('mt_enable_authorbio') ) { ?>
                            ...
                            <div class="author-thumbnail col-md-4">
                                ...
                                <div class="pull-left">
                                    <div class="author-name">
                                        <span><?php echo esc_attr__('Written by','medpluswp'); ?></span>
            ...
            <?php if(class_exists( 'ReduxFrameworkPlugin' )){ ?>
                ...
                <?php }elseif(isset($select_post_layout) && $select_post_layout == 'inherit') { ?>
                    <?php if ( medpluswp_redux('mt_single_blog_layout') == 'mt_single_blog_right_sidebar') { ?>
                        <div class="col-md-4 sidebar-content sidebar-right">
                            <?php if (is_active_sidebar($sidebar)) { ?>
                                <?php dynamic_sidebar($sidebar); ?>
                            <?php } ?>
...
<div class="row post-details-bottom">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <?php if ( medpluswp_redux('mt_enable_related_posts') ) { ?>
                ...
                <div class="related-posts sticky-posts">
                    ...
                    <?php if ($tags) { ?>
                    <h2 class="heading-bottom"><?php esc_attr_e('Related Posts', 'medpluswp'); ?></h2>
                    <div class="row">
                        <?php $tag_ids = array();  
                        ...
                        ?>  
                            <div class="vc_col-md-4 post">
                                <div class="related_blog_custom">
                                    <?php $thumbnail_src = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ),'medpluswp_related_post_pic500x300' ); ?>
                                    ...
                                    <div class="related_blog_details">
                                        ...
                                        <div class="post-author">by <a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ), get_the_author_meta( 'user_nicename' ) ); ?>"><?php echo get_the_author(); ?></a> - <?php echo get_the_date( 'j M' ); ?></div>
                                    </div>
                                </div>
                            </div>
